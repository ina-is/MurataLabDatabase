...
class AlrAnnotation():
    ...
    def annotate_intervals(self, listener_filter: str) -> List[Tuple[float, float, int, str]]:
        """
        ...
        Args:
            listener_filter (str): フィルタリングするリスナー
        Returns:
            List[Tuple[float, float, int, str]]: 10ミリ秒ごとのラベル付き時間区間のリストと応答種類
        """
        ...
        for start_time, end_time in start_end_times:
            ...
            while current_time < end_time:
                ...
                response_type = 'none'
                for resp in responses:
                    if float(resp['starttime']) == round(current_time, 2) and resp['listener'] == listener_filter:
                        label = 1
                        response_type = resp['label']
                        break
                intervals.append((current_time, next_time, label, response_type))
                current_time = next_time

        return intervals

def save_to_csv(data: List[Tuple[float, float, int, str]], filename: str):
    with open(filename, 'w', newline='') as csvfile:
        csvwriter = csv.writer(csvfile)
        csvwriter.writerow(['Start Time', 'End Time', 'Label', 'Response Type'])
...
# データをCSVファイルに保存
save_to_csv(intervals, 'annotated_intervals.csv')
